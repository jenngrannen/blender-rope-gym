...
def render_frame(frame, render_offset=0, step=1, num_annotations=400, filename="%06d_rgb.png", folder="images"):
...
def random_perturb(start_frame, params, render=False, render_offset=0):
    ...
    pick, hold, _ = find_knot(params["num_segments"], full_rope=True)
...
def take_undo_action(start_frame, pull_idx, hold_idx, norm_action_vec, render=False, render_offset=0, scale_action=True, hold_flag=True):
    ...
    for step in range(start_frame + 10, start_frame + 190 + settle_time+1):
        bpy.context.scene.frame_set(step)
        if render:
            render_frame(step, render_offset=render_offset, step=1)
            # render_frame(step, render_offset=render_offset, step=4)

    return start_frame+190+settle_time, action_vec
...
def reidemeister_right(start_frame, end1_idx, end2_idx, render=False, render_offset=0):
    ...
    middle_up_frame = start_frame+30
    middle_move_frame = middle_up_frame+80
    end_frame = middle_move_frame+30
    take_action(end1, middle_up_frame, (0,0,1))
    take_action(end1, middle_move_frame, (end1.matrix_world.translation[0]-10,0-end1.matrix_world.translation[1],0))
...
def reidemeister_left(start_frame, end1_idx, end2_idx, render=False, render_offset=0):
    ...
    end2 = get_piece(piece, end2_idx)
    take_action(end2, end_frame, (end2.matrix_world.translation[0]+4,0-end2.matrix_world.translation[1],0))
    # Drop the ends
...
def undone_check(start_frame, prev_pull, prev_hold, pull_idx, hold_idx, prev_action_vec, end1_idx, end2_idx, render_offset=0):
    piece = "Cylinder"
